...
with mp.tasks.vision.FaceLandmarker.create_from_options(face_options) as face_landmarker, \
    ...
    while cap.isOpened():
        ...
        # ---- Extract EAR and MAR (update cache independently) ----
        if latest_face_result and latest_face_result.face_landmarks:
            ...
            try:
                ...
                pass  # keep last known EAR

            try:
                last_mar = calculate_MAR(landmarks, MOUTH, w, h)
            except Exception:
                pass  # keep last known MAR

        # ---- Extract SHA ----
        if latest_holistic_result and latest_holistic_result.pose_landmarks:
            try:
                last_sha = calculate_SHA(latest_holistic_result.pose_landmarks[0], w, h)
            except Exception:
                pass  # keep last known SHA

        # ---- Use cached values or fallback to N/A ----
        ear_display = last_ear if last_ear is not None else "N/A"
        mar_display = last_mar if last_mar is not None else "N/A"
        sha_display = last_sha if last_sha is not None else "N/A"

        # ---- Display values on screen ----
        cv2.putText(frame, f"EAR: {ear_display}", (30, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.putText(frame, f"MAR: {mar_display}", (30, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
        cv2.putText(frame, f"SHA: {sha_display}", (30, 130),
